...
# Confirm the row names correspond to the gene IDs. They should, but some species need a bit of formatting here, see under species specific notes.
rownames(
    inData
)
...
# Extract those columns only from the whole scRNA-seq table, and re-set the row names to the gene IDs
...
##### DONE WITH SPONGE-SPECIFIC STUFF#####
...
namevect<-rownames(NC_cells_NC_genes) # Hydra's command is different

### MODIFICATION FOR HYDRA BELOW ###
